<template >
  <div class="min-vh-100" style="background-color: #100036;">
    <!-- Navbar -->
    <nav class="navbar navbar-expand-lg navbar-light border-bottom py-0 px-4 sticky-top" style="background-color: #350775;">
      ...
      <a class="navbar-brand fw-bold fs-4 text-white" href="/feed">
        ...
        <div class="collapse navbar-collapse" id="navbarContent">
          ...
          <ul class="navbar-nav me-auto mb-2 mb-lg-0 d-lg-flex gap-4 mx-auto">
            <!-- HOME -->
            <li class="nav-item" v-if="$route.path !== '/signup'">
              <RouterLink to="/feed" class="nav-link text-primary">
                <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24"
                  fill="none" stroke="#39ff65" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round">
          ...
          <!-- Perfil o botón Acceder -->
          <div class="d-flex align-items-center gap-3 mt-3 mt-lg-0">
            <template v-if="userStore.user.isAuthenticated && userStore.user.id">
              <RouterLink :to="{ name: 'profile', params: { id: userStore.user.id } }">
                <img v-if="userStore.user.avatar" :src="getAvatarUrl(userStore.user.avatar)" class="rounded-circle"
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      pollInterval = setInterval(getNotifications, 2000)
    })
